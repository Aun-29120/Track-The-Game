...
def interpolate_pipeline(
    ...
    for t in valid_tracks:
        ...
        if len(obs_frames) > 2:
            try:
                x_vals = obs_frames
                y_vals = [t.observations[f] for f in obs_frames]
                spline = PchipInterpolator(x_vals, y_vals)
            except Exception:
                pass
        ...
        for i in range(len(obs_frames) - 1):
            ...
            # Monotonic Spline (or Linear fallback) interpolation
            for f in range(f1, f2):
                if spline is not None:
                    interp_box = spline(f).tolist()
                else:
    ...
    if CFG.debug_logging:
        ...
        if jump_distances:
            ...
            over_threshold = 0
            for i_jd, d in enumerate(jump_distances):
                # Use the same scaling as the actual gating logic
                gap = ball_frames[i_jd+1] - ball_frames[i_jd] if i_jd+1 < len(ball_frames) else CFG.keyframe_interval
                multiplier = min(gap / CFG.keyframe_interval, 3.0)
                scaled_max = CFG.max_ball_jump_norm * multiplier
                if d > scaled_max:
                    over_threshold += 1
            print(f"[DIAGNOSTIC] Consecutive KF jump distances: avg={avg_jump:.1f}, max={max_jump:.1f}")
            print(f"[DIAGNOSTIC] Jumps > scaled_max_ball_jump: {over_threshold}/{len(jump_distances)}")
        else:
    ...
    if ball_frames:
        ...
        if len(ball_frames) > 2:
            try:
                x_vals = ball_frames
                y_vals = [ball_obs[f] for f in ball_frames]
                ball_spline = PchipInterpolator(x_vals, y_vals)
            except Exception:
                pass
        ...
        for i in range(len(ball_frames) - 1):
            ...
            for f in range(f1, f2):
                if ball_spline is not None:
                    interp_box = ball_spline(f).tolist()
                else:
                    interp_box = interpolate_box(box1, box2, f1, f2, f)
                y1, x1, y2, x2 = interp_box
